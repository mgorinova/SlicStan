...
let expand_and_filter (cs: ConstraintInfo list): ConstraintInfo list =
    ...
    let check_not_tautology_nf (c: Constraint, i: string): bool =
        ...
        | Leq(l1, l2) -> 
            if (not (l1 <= l2)) then failwith (sprintf "type level constraints cannot be satisfied:\n    %s" i)
                 ...
                 | LevelVar(_), l -> not (l = GenQuant) // doesn't work for semilattice
    ...
    let rec expand_constraint (c: Constraint): Constraint list =
        ...
        | Leq (LevelVar(x), Lub ls) -> 
            let ls' = recursive_lub_var_filtering ls 
            if List.contains GenQuant ls then [] // doesn't work for semilattice
...
let expand_and_filter_semilattice (cs: ConstraintInfo list): ConstraintInfo list =
    ...
    let rec expand_constraint (c: Constraint): Constraint list =
        ...
        (*| Leq (LevelVar(x), Lub ls) ->             
            ...
            if List.contains GenQuant ls then 
                List.map (fun li -> Leq(li, GenQuant)) ((LevelVar(x)) :: ls)
                |> List.fold ( fun s li -> List.append s (expand_constraint li)) [] 

            elif List.contains Lz ls then 
                List.map (fun li -> Leq(li, Lz)) ((LevelVar(x)) :: ls)
                |> List.fold ( fun s li -> List.append s (expand_constraint li)) []
            ...
            then
                ...
                let gr = make_ground_lub ls'
                [ Leq (LevelVar(x), gr) ]
            elif List.length ls' = 1 then [ Leq (LevelVar(x), List.head ls') ] 
            else [ Leq (LevelVar(x), Lub ls') ] *)
